...
createDatamatrix <- function(locus,knownGenos,idsU=NULL){
  ...
  if(nU > 0){
    ...
    grid.subset <- as.matrix(grid.subset) # faster as matrix
    nr <- nrow(grid.subset)

    #Then, look up the genotype combinations in allgenos
    #to find the actual genotype

    #Preallocate matrix (avoid growing with cbind)
    datamatrixU <- matrix(0, nrow = nU, ncol = 2*nr)

    #Populate two columns at a time
    for(i in seq_len(nr)){
      datamatrixU[, c(2*i-1, 2*i)] <- allgenos[grid.subset[i,], , drop=FALSE]
    }

    #Convert back to original allele names
    datamatrixU[] <- origAlleles[datamatrixU]
